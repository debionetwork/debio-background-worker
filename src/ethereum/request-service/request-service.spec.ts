...
describe('Request Service Ethereum', () => {
  ...
  beforeAll(async () => {
    ...
    requestServiceController = module.get<RequestServiceController>(
      RequestServiceController,
    );
    requestServiceService = module.get<RequestServiceService>(
      RequestServiceService,
    );
    setLastRequestServiceBlockHandler =
      module.get<SetLastRequestServiceBlockHandler>(
        SetLastRequestServiceBlockHandler,
      );
    getLastRequestServiceBlockHandler =
      module.get<GetLastRequestServiceBlockHandler>(
        GetLastRequestServiceBlockHandler,
      );
    createServiceRequestHandler = module.get<CreateServiceRequestHandler>(
      CreateServiceRequestHandler,
    );
  });
  ...
  describe('Ethereum block request', () => {
    it('Set Last Request Service Block Handler', async () => {
      ...
      const setLastRequestServiceBlockHandlerSpy = jest.spyOn(
        setLastRequestServiceBlockHandler,
        'execute',
      );
      const setLastRequestServiceBlockCommand: SetLastRequestServiceBlockCommand =
        new SetLastRequestServiceBlockCommand(lastBlockNumber);
      await setLastRequestServiceBlockHandler.execute(
        setLastRequestServiceBlockCommand,
      );
      expect(setLastRequestServiceBlockHandlerSpy).toBeCalled();
      expect(setLastRequestServiceBlockHandlerSpy).toBeCalledWith(
        setLastRequestServiceBlockCommand,
      );
    });

    it('Get Last Request Service Block Handler', async () => {
      const createServiceRequestHandlerSpy = jest.spyOn(
        getLastRequestServiceBlockHandler,
        'execute',
      );
      const res = await getLastRequestServiceBlockHandler.execute();
      expect(createServiceRequestHandlerSpy).toBeCalled();
      expect(res).toBe(1234);
      expect(createServiceRequestHandlerSpy).toHaveReturnedWith(
        expect.any(Number),
      );
    });
    ...
    it('Create Service Request Handler', async () => {
      const mockArgs = ['', '', '', '', '', '', '', '', ''];
      const mockBlockNumber: BlockMetadata = {
        blockHash: '',
        blockNumber: 12345,
        transactionHash: '',
      };
      ...
      const createServiceRequestCommand: CreateServiceRequestCommand =
        new CreateServiceRequestCommand(mockArgs, mockBlockNumber);
      await createServiceRequestHandler.execute(createServiceRequestCommand);
      expect(createServiceRequestHandlerSpy).toBeCalled();
      expect(createServiceRequestHandlerSpy).toBeCalledWith(
        createServiceRequestCommand,
      );
    });
  });
